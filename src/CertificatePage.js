...
import certificate from './Pages/Trainers/certificateimage.png'
...
function CertificatePage() {
  ...
  const generatePDF = () => {
    const element = document.getElementById('cert-div'); // Replace 'divId' with the ID of your div element
  
    const pdfOptions = {
      filename: `${stud}.pdf`,
      image: { type: 'jpeg', quality: 0.98 },
      html2canvas: { scale: 4 },
      jsPDF: { unit: 'px', format: [310, 540], orientation: 'landscape' },
    };
  
    html2pdf().from(element).set(pdfOptions).save();

    // html2pdf().from(element).save(`${stud}.pdf`);
  };
          ...
          <Box className={classes.box}>
            <Grid container spacing={4} alignItems="center">
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Button padding={1} onClick={generatePDF} variant="contained" color="primary" fullWidth>
                  Download
                </Button>
