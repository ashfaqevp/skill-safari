...
const EditAdmin = (props) => {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        const docRef = doc(db, "admins/" ,props.id,);
      ...
      const handleSubmit = async (e) => {
        ...
        if (name && email && phone) {
          try {
            ...
            const userRef = doc(db, "admins/" , props.id );
            const newUser = {
              name: name ? name : "",
              email: email ? email : "",
    ...
    <label htmlFor="image-upload" style={{ position: "absolute", bottom: "-15%", right: "-15%", display: "flex", justifyContent: "center", alignItems: "center", width: "40%", height: "40%", borderRadius: "50%", backgroundColor: "#fff", boxShadow: "0px 2px 6px rgba(0, 0, 0, 0.3)", cursor: "pointer" }}>
      ...
      <Grid item xs={9.5}>
        ...
        <Grid container justifyContent="flex-end">
            <Grid item>
              <Button endIcon={<PersonAdd/>} style={{marginTop:"20px"}} variant="contained" onClick={handleSubmit}>Edit Admin</Button>
            </Grid>
        </Grid>
