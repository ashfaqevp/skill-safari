...
const StudentProjects = () => {
  ...
  const getProjects = async () => {
    const projectsRef = collection(db, `batches/${id}/students/${std}/certificates`);
  ...
  const handleDeleteConfirm = async () => {
    try {
        ...
        const studentRef = doc(db, `batches/${id}/students/${std}/certificates/${projectId}`);
  ...
  return (
    <div className={classes.root}>
      ...
      <Box className={classes.batchBox}>
        ...
        <Typography variant="h6"><b>Certificates ({projects.length})</b></Typography>
  ...
  <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
    ...
    <DialogContent>
    <AddCertificate handleSubmit={handleSubmit} id={id} std={std} />
    </DialogContent>
  </Dialog>
