...
const AddProjects = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const storageRef = ref(storage, `badges/${image.name}`);
    ...
    try {
      const projectsRef = collection(db, `batches/${props.id}/students/${props.std}/badges`);
      const newProject = {
        title: title,
    ...
    <Box className={classes.container}>
      ...
      <Grid container spacing={2} >
        <Grid item xs={12} sm={12}>
          <TextField label="Badge Name"  onChange={(e) => setTitle(e.target.value)}  fullWidth className={classes.inputField} />
        </Grid>

        <Grid item xs={12}>
          <TextField label="Badge Description" onChange={(e) => setDescription(e.target.value)}  fullWidth multiline rows={4} className={classes.inputField} />
        </Grid>
          ...
          <Grid container justifyContent="flex-end">
            <Grid item>
              <Button endIcon={<AddCard/>} style={{marginTop:"20px"}} variant="contained" onClick={handleSubmit}> Add Badge </Button>
            </Grid>
          </Grid>
