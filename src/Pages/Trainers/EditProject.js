...
const AddProjects = (props) => {
  ...
  const fetchData = async () => {
    const docRef = doc(db, `batches/${props.id}/students/${props.std}/projects/${props.project}`);
  ...
  const handleSubmit = async (e) => {
    ...
    if(title && category && description && github && live && image ){
      try {
        ...
        await updateDoc(docRef, newProject,  { merge: true });
      ...
      <Grid container spacing={2} >
        ...
        <Grid item xs={12} sm={6}>
          ...
          <Grid container justifyContent="flex-end">
            <Grid item>
              <Button endIcon={<AddCard/>} style={{marginTop:"20px"}} variant="contained" onClick={handleSubmit}>Edit Project</Button>
            </Grid>
          </Grid>
